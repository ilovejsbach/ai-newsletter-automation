...
Info "  3) deps:    uv sync"
Info "  4) keys:    copy .env.example .env ; notepad .env   (set OPENAI_API_KEY)"
Info "  5) verify:  uv run ai-newsletter sample"
Info "  App TLS (crawl/OpenAI) uses the OS trust store automatically (truststore)."
Info "  Only if you still get SSL errors, run the fallback: scripts\setup_windows.ps1"
